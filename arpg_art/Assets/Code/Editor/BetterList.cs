...
public class BetterList<T>
{
	...
	public void Reserve (int miniCapacity)
	{
		if (null != buffer)
		{
			var nextSize = buffer.Length;

			if (nextSize < miniCapacity)
			{
				do 
				{
					nextSize <<= 1;
				}
				while(nextSize < miniCapacity);

				var nextList = new T[nextSize];
				if (size > 0)
				{
					buffer.CopyTo(nextList, 0);
				}
	...
	{
		if (null != items && count > 0 && items.Length >= count)
		{
			Reserve(size + count);
			System.Array.Copy(items, 0, buffer, size, count);
			size += count;
		}
	}
