...
{
	[MenuItem("Build/BuildAnimaiton %#b")]
	...
	static AnimationClip BuildAnimationClip(DirectoryInfo dictorys)
	{
		...
		AssetDatabase.CreateAsset(clip,_animationPath +"/"+parentName +"/" +animationName+".anim");
	...
	{
		AnimatorController animatorController = AnimatorController.CreateAnimatorControllerAtPath(_animationControllerPath +"/" + name + "/" +name+".controller");
	...
	static void BuildPrefab(DirectoryInfo dictorys,AnimatorController animatorCountorller)
	{
		...
		var temp  = dictorys.GetDirectories();

		FileInfo images = temp[0].GetFiles("*.png")[0];
		...
		PrefabUtility.CreatePrefab(_prefabPath+"/"+ go.name + "/" + go.name+".prefab",go);
		DestroyImmediate(go);
	}
	...
	private static string _prefabPath = "Assets/prefabs/animator";
	private static string _animationControllerPath = "Assets/prefabs/animator";
	private static string _animationPath = "Assets/prefabs/animator";
	private static string _imagePath = "Assets/share/atlas/animator";
}