...
//		}
    ...
    {
        //进入内圈然后，切换玩家。有可能切换进入内圈，切换玩家，当前的玩家再掷筛子
        /*
         * 1、在玩家进入内圈时掉线
         * 2、在其他玩家在进入内圈的时候接入游戏
         * 3、在其他玩家掷筛子时进入游戏
        ...
        if(null!=_dictRoom[roomID])
        {
            if(_dictRoom[roomID].getCuurrentStateType()== Server.UnitFSM.FSMStateType.UpGradState)
            {
            ...
            {
                Client.Unit.BattleController.Instance.EnterInner_Direct();
...
//		_instance = null;
	...
	/// <summary>
	/// Nets the game lost to next one.轮到当前玩家，但是当前玩家掉线了，自动到下一个玩家
	/// </summary>
