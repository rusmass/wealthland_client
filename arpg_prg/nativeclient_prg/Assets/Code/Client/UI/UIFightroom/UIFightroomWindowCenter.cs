...
namespace Client.UI
{
	public partial class UIFightroomWindow
	{
		...
		private void _showCenter()
		{
            ...
            GameModel.GetInstance.playNetMode = 0;

            GameModel.GetInstance.HideNetLoading ();

            if(GameGuidManager.GetInstance.DoneRoom==false)
            {
                GameGuidManager.GetInstance.ShowFightRoomGuid();
            }
        ...
        /// 
        /// </summary>
        private void _hideNewPlayerGuid()
        {
            if(GameGuidManager.GetInstance.DoneRoom==false)
            {
                GameGuidManager.GetInstance.DoneRoom = true;
            }
        }
