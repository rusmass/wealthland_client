...
//		}
		...
		private void _ShowChangeBoard(bool isBuy=false)
		{
			...
			if(_isBuyShare == false)
			{
                ...
                _sharecode.SetActiveEx(true);
                _scrollview.SetActiveEx(true);
			...
			{
                _buyShareItem.SetActiveEx(true);
                _controller.IsBuyShare(isBuy);
		...
		private void _SureCloseHandler(GameObject go)
		{
            ...
            this.img_chagne.SetActiveEx(false);
            _normalBoard.SetActiveEx(true);
            _SetClockPositionForInit();
            lb_inputShare.text = "0";
		...
		private void _SureChangeHandler(GameObject go)
		{
			...
			if (_isBuyShare == false)
			{
               // Console.Error.WriteLine("当前股票组件：hahah");
