...
namespace Client.UI
{
    public partial class UIEnterSuccessWindow
    {
        private void _InitTop(GameObject go)
        {
            ...
            lb_innertime = go.GetComponentEx<Text>(Layout.lb_innertime);
        ...
        private void _OnShowTop()
        {
            ...
            var innerTime = GameTimerManager.Instance.getInnerTime(_controller.playerInfor.playerID);
            var innerTimeTip = "您在内圈的游戏时间是:"+innerTime ;
            if (PlayerManager.Instance.IsHostPlayerTurn() == false)
            {
                innerTimeTip = String.Format("{0}在内圈的游戏时间是:{1}",_controller.playerInfor.playerName,innerTime);
                tipString = string.Format("恭喜<color=#e94444ff>{0}</color>获得胜利", _controller.playerInfor.playerName);
            }
            _titleTxt.text = tipString;
            this.lb_innertime.text = innerTimeTip;
