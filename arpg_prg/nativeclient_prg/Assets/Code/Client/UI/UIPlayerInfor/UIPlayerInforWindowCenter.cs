...
namespace Client.UI
{
	public partial class UIPlayerInforWindow
	{
		...
		private void _InitCenter(GameObject go)
		{
			...
			img_selecthead = go.GetComponentEx<Image> (Layout.btn_headselect);
...
//			_imgTitle =new UIImageDisplay( go.GetComponentEx<Image> (Layout.img_title));
		...
		private void _ShowCenter()
		{
            ...
            //Console.Error.WriteLine("选择头像的按钮不可见："+(btn_selecthead==null).ToString());
...
//			img_selecthead.Load (headpath);
		...
		/// <summary>
		/// Sets the head path.设置人物头像路径
		/// </summary>
		/// <param name="value">Value.</param>
		public void SetHeadPath()
		{
			headpath = GameModel.GetInstance.myHandInfor.headImg;
			AsyncImageDownload.Instance.SetAsyncImage (headpath,img_selecthead);
//			DownLoadServerPicture.Instance.SetAsyncImage(headpath,img_selecthead);
		}
		...
		private void _OnSureHandler(GameObject go)
		{
			...
			data["avatar"]=this.headpath;
			data["nick"]=input_name.text ;
			data ["account"] = GameModel.HttpAcount;
