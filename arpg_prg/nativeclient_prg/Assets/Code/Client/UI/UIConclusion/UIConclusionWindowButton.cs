...
namespace Client.UI
{
	public partial class UIConclusionWindow
	{
		private void _OnInitButton(GameObject go)
		{
            ...
            _btnDetail = go.GetComponentEx<Button>(Layout.btn_showDetail);
		...
		private void _OnShowButton()
		{
            ...
            EventTriggerListener.Get(this._btnDetail.gameObject).onClick += _ShowConclsionHandler;

            this._btnDetail.SetActiveEx(true);
        ...
        /// </summary>
        /// <param name="go"></param>
        private void _ShowConclsionHandler(GameObject go)
        {
            var tmpController = UIControllerManager.Instance.GetController<UIConclusionDetailController>();
            tmpController.player = _controller.player;
            tmpController.totalScoreTip = this.letter;
