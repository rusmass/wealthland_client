...
namespace Client.UI
{
	public partial class UIBattleWindow
	{
		...
		public void ShowStayRoundTip()
		{
            ...
            {
                var tmpPlayer = PlayerManager.Instance.HostPlayerInfo;
                var showTip = false;
                if(tmpPlayer.IsTipGameRound())
                {
                    showTip = true;
                }
                ...
                {
                    HideStayRoundTip();
                    var tipController = UIControllerManager.Instance.GetController<UIGameAutoTipController>();
                    tipController.LeftTime = 3;
                    tipController.GameTip =string.Format("已经过去{0}年了，您现在{1}岁。当前领先的游戏玩家是:{2}",tmpPlayer.CircleNumber(),tmpPlayer.totalAge,PlayerManager.Instance.FastPlayerInfor().playerName);
                    tipController.setVisible(true);
                    tmpPlayer.UpdateOldRound();
                    HideControllerBoard();
                    return;
                }              
            }
