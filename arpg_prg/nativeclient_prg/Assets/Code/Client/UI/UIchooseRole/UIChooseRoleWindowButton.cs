...
namespace Client.UI
{
	public partial class UIChooseRoleWindow 
	{
		private void _OnInitButton(GameObject go)
		{
            ...
            _btnCreate = go.GetComponentEx<Button>(Layout.btn_creat);
		...
		private void _OnShowButton()
		{
            ...
            {
                GameGuidManager.GetInstance.ShowSingleSelectGuid();
            }
            ...
            EventTriggerListener.Get(_btnCreate.gameObject).onClick += _OnClickCreatHandler;
...
//			}
        ...
        private void _OnClickCreatHandler(GameObject go)
        {
            //Console.Error.WriteLine("啦啦啦啦啦");            
            var newRoleController = UIControllerManager.Instance.GetController<UISelfChooseController>();            
            newRoleController.SetPlayerInfo(_controller.SelectRole(roleIndex), roleIndex);
            newRoleController.setVisible(true);
...
//}
		...
		private void _OnBtnLookClick(GameObject go)
		{
            ...
            Tweener tweener = _boardBill.transform.DOLocalMoveX(-40, 1f);
		...
		{
			Tweener tweener = _boardBill.transform.DOLocalMoveX(320,1f);
			//设置这个Tween不受Time.scale影响
			tweener.SetUpdate(true);
			//设置移动类型
			tweener.SetEase(Ease.Linear);
