...
namespace Client.UI
{
	public partial class UIFightCatchWindow
    {
		private void _initCenter(GameObject go)
		{
			...
			for (int i = 1; i < 5; i++)
			{
				...
				Image tmpHead = tmpImg.gameObject.GetComponentEx<Image> ("headimg");
		...
		private void _showCenter()
		{
            ...
            _timeCount = new Timer(1000);
            _timeCount.Elapsed += new ElapsedEventHandler(_OnTimeHandler);
            _timeCount.AutoReset = true;
            _timeCount.Enabled = true;           

            //
            this._SetLeftTime(this._leftTime);
            _timeCount.Start();
        }
        ...
        /// 计算倒计时的时间
        /// </summary>
        private void _OnTimeHandler(object obj, System.Timers.ElapsedEventArgs e)
        {
            this._leftTime--;            
        }


        private void _OnTickCenter(float deltime)
        {
            if (_leftTime >= 0)
            {
                //Console.Error.WriteLine("当前剩余的时间:" + this._leftTime);
                this._SetLeftTime(this._leftTime);
            }
            else
            {
                if(_isAutoAgree==false)
                {
                    NetWorkScript.getInstance().MatchEnterGame(GameModel.GetInstance.myHandInfor.uuid);
                    this._StopTimeHandler();
                    _isAutoAgree = true;
                }
        ...
        private void _SetLeftTime(int value)
        {
            if(null!=this.lb_time)
            {
                this.lb_time.text = value.ToString();
            }           
        }
...
//		private List<UIImageDisplay> headList = new List<UIImageDisplay> ();
		private List<Image> headList = new List<Image> ();
        ...
        /// 是否是自动同意
        /// </summary>
        private bool _isAutoAgree=false;
