...
namespace Client.UI
{
	public partial class UIBorrowWindow
	{
		...
		{			
            ...
            btn_borrowFriend = go.GetComponentEx<Button>(Layout.btn_borrowFriend);
		...
		private void _OnShowCenter()
		{
            ...
            EventTriggerListener.Get(btn_borrowFriend.gameObject).onClick += _BorrowFriendHandler;

            if(GameModel.GetInstance.isPlayNet==false)
            {
                btn_borrowFriend.SetActiveEx(false);
            }
            else
            {
                btn_borrowFriend.SetActiveEx(true);
            }
...
//				btn_payback.SetActiveEx (false);
        ...
        /// 隐藏还款的新手引导
        /// </summary>
        private void _HidePaybackGuid()
        {
            if(null!=this._borrowPayBack)
            {
                if(this._borrowPayBack.IsShowNewPlayerGuid())
                {
                    if(GameGuidManager.GetInstance.DoneGamePayback==false)
                    {
                        GameGuidManager.GetInstance.DoneGamePayback =true;
                    }
                }
            }
        }
...
//				canborrow = player;
        ...
        private Button btn_borrowFriend;
