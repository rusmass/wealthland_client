...
//			return;
		...
		/// <summary>
		/// Updates the sure infor. 更新确定选择人物的信息
		/// </summary>
		...
		public void UpdateSureInfor(JsonData value)
		{
			for (var i = 0; i < _chooseVoList.Count; i++)
			{
				...
				var tmpBoo = bool.Parse (value [tmpVo.playerId].ToString ());

				if (tmpBoo == false)
				{
					_readyImgList [i].SetActiveEx (false);
				}	
				else
				{
					_readyImgList [i].SetActiveEx (true);
		...
		public void UpdateSelectInfor(JsonData roleData , JsonData playerData,JsonData readyStatus)
        {
			//角色选择数据  角色id：角色是否是被选中  遍历左侧，显示哪些角色被选中.如果是被自己选中，图片加高亮
			//玩家选择信息playerId : 角色id ， 是否是准备好，遍历右侧显示玩家的选择角色
			//如果是自己选择的角色，就显示头像，如果自己未选择，就不显示
			for (var i = 0; i < _initPlayerInitDatas.Count; i++)
			{
				var tmpvo = _initPlayerInitDatas [i];
				if (bool.Parse(roleData [tmpvo.id.ToString()].ToString())==true)
				{
					_selectedState [i] = 1;
					var slefChoose=int.Parse(playerData[GameModel.GetInstance.myHandInfor.uuid].ToString());
					if (slefChoose == tmpvo.id)
					{
						_selectedState [i] = 2;
					}
				}
				...
				{					
					...
					if (null != _controller) 
					{
						_playerData = _controller.SelectRole(i);
						_OnShowHeroInfor(_playerData);
					}
				}

				if (_selectedState [i] == 0)
				{
				...
				else
				{
					...
					_tmpRoleNameBg.SetActiveEx (false);
				}

				var tmpChooseVo = _chooseVoList[i];

				var tmpPlayerChoose = playerData[tmpChooseVo.playerId];

				var roleId=int.Parse(tmpPlayerChoose.ToString());

				if (roleId > 0)
				{
					tmpChooseVo.careerId=roleId;
					for (var j = 0; j < _chooseVoList.Count; j++)
					{
						var tmpPlayerdata = _controller.SelectRole(j);
						if (tmpPlayerdata.id == roleId)
						{
