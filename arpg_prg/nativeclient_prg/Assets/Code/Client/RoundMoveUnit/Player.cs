...
namespace Client.Unit
{
    public class Player : BattleUnit
    {
        ...
        {
            //Console.Error.WriteLine("开始进入内圈了啊啊啊啊啊");
            _initPoint = innerInitPoint;
            _currentPos = _initPoint;
            _wayPoint = _wayPointInner;
            _blockType = BlockType.Inner;
            _root.transform.localScale = Vector3.one * 1.4f;
            //Console.Error.WriteLine("当前初始化的位置是："+_initPoint.ToString()+"______currentPos:"+ _currentPos+"____:"+ _wayPoint.ToString());
            _InitPosAndRot();
        }
        ...
        public void enterInnerShow()
        {
            ...
            tweener = _root.transform.DOScale(Vector3.zero, 2f);
        ...
        private void setPlayerScale()
        {
            ...
            tweener = _root.transform.DOScale(new Vector3(1.4f, 1.4f, 1.4f), 2f);
            //设置这个Tween不受Time.scale影响
            tweener.SetUpdate(true);
            //设置移动类型
            tweener.SetEase(Ease.Linear);
        ...
        void InnerEvent()
        {
            ...
            //GameTimerManager.Instance.EnterInnerTime(player.playerID);
