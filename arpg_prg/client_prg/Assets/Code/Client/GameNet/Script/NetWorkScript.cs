...
public  class NetWorkScript 
{
    ...
    //	public static string host="59.110.29.41";192.168.1.156
    //192.168.1.153   219.232.230.61   156 
    public static string host = "118.144.74.82";//"219.232.230.61";

	public int port = 8069; //8066     8067   8068苹果商城的   8069 6.5内测版本的
...
//		Console.Warning.WriteLine (chatStr);
...
//		Debug.LogWarning("购买卡牌了---"+((int)Protocol.Game_BuyCard).ToString());
		msg += endSymbol;
		sendMessage (msg);
	}
	...
	public void QuitCard(string roomId,int cardId , int cardType)
	{
		...
		sendData ["body"] = badyData;
		var msg = sendData.ToJson ();
//		Debug.LogWarning("放弃购买卡牌了---"+((int)Protocol.Game_BuyCard).ToString());
		msg += endSymbol;
		sendMessage (msg);
	}
...
//			int card = tmpSaleVo.cardId;
	...
	public void Game_BorrowMoney(string roomId ,List<BorrowVo> _data,int loanType )
	{
		...
		badyData ["roomId"] = roomId;
//		badyData ["loanType"] = loanType;
		var tmpData = new JsonData ();
		for (var i = 0; i < _data.Count; i++)
		{			
			var borrowData=_data[i];
//			var tmpstr = Coding<BorrowVo>.encode (borrowData);
